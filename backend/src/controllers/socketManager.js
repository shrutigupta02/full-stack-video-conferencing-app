    ...
    socket.on("join-call", (path) => {
      ...
      for (let a = 0; a < connections[path].length; a++) {
        io.to(connections[path][a]).emit(
          ...
          socket.id,
          connections[path]
        ); //notifies all people in room that user joined
      }
    ...
    socket.on("chat-message", (data, sender) => {
      ...
      if (found === true) {
        if (messages[matchingRoom] === undefined) {
          //checks if there are any messages in the room, if not it initialises an empty array
        ...
        connections[matchingRoom].forEach((elem) => {
          //notifies every person in room of new message
